...
**Example integration tests** (requires live AWS + OCM credentials): to test a single example during development, use `scripts/run-example.sh <example_name>` (requires `RHCS_TOKEN` and `TF_VAR_cluster_name`). For **pre-release validation** of all examples, use `scripts/run-all-examples.sh` which runs `terraform apply` + `destroy` across every example. Use `--dry-run` to validate prerequisites, `--include`/`--exclude` to select examples, and `--cluster-prefix` to namespace clusters. Always set `--cluster-prefix` to something identifiable (e.g. your name or ticket) so your test clusters are easy to find and clean up. Requires `RHCS_TOKEN`; shared-vpc examples also need dual-account AWS credentials (see `scripts/run-all-examples.sh --help`). To run against the staging environment, set `RHCS_URL` (e.g. `export RHCS_URL=https://api.example.com`).
