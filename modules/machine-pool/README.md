...
| <a name="input_aws_node_pool"></a> [aws\_node\_pool](#input\_aws\_node\_pool) | Configures aws settings for the pool. | <pre>object({<br/>    instance_type                   = string<br/>    tags                            = map(string)<br/>    additional_security_group_ids   = optional(list(string))<br/>    capacity_reservation_id         = optional(string)<br/>    capacity_reservation_preference = optional(string)<br/>    node_drain_grace_period         = optional(number)<br/>    use_spot_instances              = optional(bool)<br/>    max_spot_price                  = optional(number)<br/>  })</pre> | n/a | yes |
