...
module "vpc" {
  ...
  proxy_user_data_file     = file("../../assets/proxy-host-user-data.sh")
}

resource "tls_private_key" "work_pk" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "aws_key_pair" "work_ssh_key" {
  provider   = aws.network-owner
  key_name   = "${var.cluster_name}-work-ssh-key"
  public_key = tls_private_key.work_pk.public_key_openssh
}

resource "local_file" "work_private_ssh_key" {
  filename        = "${aws_key_pair.work_ssh_key.key_name}.pem"
  content         = tls_private_key.work_pk.private_key_pem
...
resource "aws_instance" "host" {
  ...
  security_groups = [aws_security_group.worker_host_ingress.id]

  key_name = aws_key_pair.work_ssh_key.key_name

  tags = {
    Name = "${var.cluster_name}-work-host"
  }
}
